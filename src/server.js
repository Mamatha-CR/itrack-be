  ...
  try {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const updates = {};
        if (colorMatch && row.job_status_color_code !== colorMatch[1]) updates.job_status_color_code = colorMatch[1];
        if (orderMatch && row.job_status_order !== orderMatch[1]) updates.job_status_order = orderMatch[1];
